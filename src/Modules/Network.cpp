...
int foldersInDirectory(const std::string &path) {
  return std::distance(
      ...
      std::filesystem::end(std::filesystem::directory_iterator(path)));
}
...
void setWifiEthSysfs() {
  ...
  for (const auto &i : std::filesystem::directory_iterator("/sys/class/net/")) {
    for (const auto &j :
         std::filesystem::directory_iterator(i.path().string())) {
...
std::string getNetworkStatus(const char *networkName) {
  ...
  if (FileUtils::doesFileExist(wifiOperstate) &&
      FileUtils::readFirstLine(wifiOperstate) == "up") {
    output =
        showNetworkName ? std::string(" ") + networkName : std::string("");
  } else if (FileUtils::doesFileExist(ethOperstate) &&
             FileUtils::readFirstLine(ethOperstate) == "up") {
    output =
        showNetworkName ? std::string(" ") + networkName : std::string("");
  } else {
    output = std::string("\uf00d");
...
char *getNetworkName() {
  ...
  ret = g_dbus_proxy_get_cached_property(proxy, "ActiveConnections");
  ...
  if (paths[0] != NULL) {
    ...
    ret = g_dbus_proxy_get_cached_property(
        g_dbus_proxy_new_for_bus_sync(
            ...
            "org.freedesktop.NetworkManager", activeConnectionDbusPath,
